...
## 1. **准备工作**  
#### 1.1. 环境准备  
&emsp;&emsp;在确定了要使用的硬件平台的情况下，可以直接移植SimpleGUI的演示程序到目标平台上。在本文档中将以STM32F1芯片平台为例，讲解如何快速简要的将Simple GUI的Demo程序移植到预期的目标平台上。  
...
&emsp;&emsp;由于演示工程中包含GB2312的字库，所以编译后体积较大，所以请大家在移植时，最好使用STM32F103VE/ZE芯片，以确保Flash资源的充足。当然，这并不是硬性需求，仅仅是出于快速上手的考虑，如果您确实需要在少资源的芯片中运行，后续将有介绍如何使Demo运行于诸如STM32F103C8这样的资源相对较少的芯片，毕竟SimpleGUI本身对系统资源的消耗并不高。  
